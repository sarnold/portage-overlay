...
EAPI=5
inherit eutils db flag-o-matic java-pkg-opt-2 autotools toolchain-funcs multilib multilib-minimal versionator
...
multilib_src_configure() {
	...
	#470634 (needs more work if ld.gold and lto is default, see below)
	tc-ld-disable-gold
	...
	# 470634 (needs more work if ld.gold and lto is default, see below).
	# Also add linker versions to the symbols. Easier to do, and safer
	# than header file mumbo jumbo. This will barf with ld.gold if
	# -flto is there, and the env trick using -fno-lto does not work
	# here :/
