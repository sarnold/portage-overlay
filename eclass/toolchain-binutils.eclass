...
tc-binutils_apply_patches() {
	...
	if ! use vanilla ; then
		if [[ -n ${PATCHVER} ]] ; then
			...
			if version_is_at_least 2.26 && ! version_is_at_least 2.27 ; then
#				if [[ ${CTARGET} == avr* ]] ; then
					EPATCH_EXCLUDE+=" 00_all_0002-ld-Add-a-linker-configure-option-enable-relro.patch"
					cp "${FILESDIR}"/avr-${PN}-2.26-atmel-big-diff2.patch \
						${WORKDIR}/patch/00_all_0000-avr-${PN}-2.26-atmel-big-diff2.patch
#				fi
			fi
...
toolchain-binutils_src_configure() {
	...
	if use cxx ; then
		...
		if is_cross ; then
			case ${CTARGET} in
			avr*) myconf+=( --with-gnu-ld --with-gnu-as --with-dwarf2 --enable-lto ) ;;
			esac
		fi
	fi
