...
src_prepare() {
	sed -i -e "s|lQtCore |lQtCore \-lQtDBus |" makefile.linux
}

src_compile() {
	cd "${MY_P}"
	MKV_DEFS="-Os -D_GNU_SOURCE -D_linux_"
	emake GCC="$(tc-getCC) ${MKV_DEFS} ${CFLAGS} ${LDFLAGS}" \
		-f makefile.linux || die "make failed"
}
