...
src_prepare() {
	...
	# update for new freetype2 header path
	if use truetype ; then
		if has_version =media-libs/freetype-2.5.1 ; then
			sed -i -e 's|freetype/freetype.h|freetype2/freetype.h|' "${S}"/configure
			sed -i -e 's|<freetype|<freetype2|' "${S}"/libavfilter/vf_drawtext.c
		else
			sed -i -e 's|freetype/freetype.h|freetype2/freetype/freetype.h|' "${S}"/configure
		fi
	fi
