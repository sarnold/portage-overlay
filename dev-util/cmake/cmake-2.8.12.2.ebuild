...
PATCHES=(
	...
	"${FILESDIR}"/${P}-hppa-bootstrap.patch
	"${FILESDIR}"/${P}-find-tinfo.patch
)
...
src_configure() {
	...
	if use ncurses && built_with_use sys-libs/ncurses tinfo ; then
		# it finds the stupid forms library but not the term library
		# even with the patch.  grrr...
		sed -i -e s"|-lform|-lform -ltinfo|" \
			${S}_build/Source/CMakeFiles/ccmake.dir/link.txt
	fi
}
