...
IUSE="amqp caps dbi geoip ipv6 json mongodb pacct +pcre redis smtp spoof-source ssl systemd tcpd"
RESTRICT="test"
...
src_install() {
	...
	dodoc AUTHORS NEWS contrib/syslog-ng.conf* contrib/syslog2ng \
		"${FILESDIR}/3.4/syslog-ng.conf.gentoo.hardened" \
	...
	if use userland_BSD ; then
		newins "${FILESDIR}/3.4/syslog-ng.conf.gentoo.fbsd" syslog-ng.conf
	else
		newins "${FILESDIR}/3.4/syslog-ng.conf.gentoo" syslog-ng.conf
	fi

	insinto /etc/logrotate.d
	newins "${TMPDIR}/syslog-ng.logrotate" syslog-ng

	newinitd "${FILESDIR}/3.4/syslog-ng.rc6" syslog-ng
	newconfd "${FILESDIR}/3.4/syslog-ng.confd" syslog-ng
