...
IUSE="+alsa avahi +caps jack lirc oss tcpd +X +no-optical-raw hal dbus libsamplerate gnome bluetooth +asyncns +glib test doc +udev ipv6 system-wide realtime"
...
src_prepare() {
	...
	use no-optical-raw && epatch "${FILESDIR}"/${PN}-analog-optical.patch
	elibtoolize
}
