...
IUSE="+alsa avahi +caps jack lirc oss +no-optical-raw tcpd +X dbus libsamplerate gnome bluetooth +asyncns +glib test doc +udev ipv6 system-wide realtime"
...
src_prepare() {
	...
	# update sb and audigy output default
	if use no-optical-raw ; then
		epatch "${FILESDIR}"/${PN}-analog-optical.patch
	fi
