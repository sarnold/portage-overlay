...
src_prepare() {
	# fix stupid source file in OBJ list
	epatch "${FILESDIR}"/${P}-fix-broken-makefile.patch
...
src_configure() {
	...
	emake DEBUG="" CC="$(tc-getCC)" OPT="${CFLAGS}" CCARGS="${mycc}" \
		AUXLIBS="${mylibs} -lresolv" makefiles
}
