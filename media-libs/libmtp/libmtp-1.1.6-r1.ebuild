...
inherit autotools eutils flag-o-matic udev user toolchain-funcs
...
src_prepare() {
	epatch "${FILESDIR}"/${P}-fbsdlibusb.patch \
		...
		"${FILESDIR}"/${P}-dereferencing_type-punned_pointer.patch
	...
	sed -i -e "s|_BSD_SOURCE|_DEFAULT_SOURCE|" src/ptp.c
}

src_configure() {
	append-cppflags "-DHAVE_STRNDUP"
