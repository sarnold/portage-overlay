...
# Note gtk-doc is currently broken
...
src_prepare() {
	...
	# Add missing Nikon Coolpix L22 ptp camera device
	epatch "${FILESDIR}"/${P}-add-missing-nikon-L22.patch
...
src_configure() {
	...
	# Note: due to gtk-doc borkage, only Doxygen docs should be
	# built with USE=doc (so do not set --enable-docs)
	use doc || export HAVE_DOXYGEN_FALSE
	econf \
		--disable-gp2ddb \
