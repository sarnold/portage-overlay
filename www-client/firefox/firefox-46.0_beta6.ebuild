...
IUSE="bindist egl hardened +hwaccel neon pgo selinux +gmp-autoupdate test"
...
src_configure() {
	...
	if use neon ; then
		...
		mozconfig_annotate '' --enable-png-arm-neon-support=nocheck
	fi

	if [[ ${CHOST} == armv* ]] ; then
		mozconfig_annotate '' --with-arm-kuser
		mozconfig_annotate '' --with-float-abi=toolchain-default
