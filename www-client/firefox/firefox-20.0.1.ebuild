...
src_prepare() {
	...
	if use webrtc ; then
		epatch "${FILESDIR}"/${P}-pcm16b.patch
	fi
...
src_configure() {
	...
	# webrtc build is now disabled on big endian arches (eg, ppc) see upstream
	...
	CC="$(tc-getCC)" CXX="$(tc-getCXX)" LD="$(tc-getLD)" \
		...
		emake -f client.mk configure || die "real configure failed"
}
