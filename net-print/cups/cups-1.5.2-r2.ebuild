...
IUSE="acl dbus debug +filters gnutls java +jpeg kerberos ldap pam pdf perl php +png python slp +ssl static-libs +threads +tiff usb X xinetd"
...
pkg_setup() {
	...
	if use pdf; then
		ewarn ""
		ewarn "With USE=pdf enabled, you must install cups-filters"
		ewarn "for full print functionality.  These filters will be"
		ewarn "mandatory for >=cups-1.6"
		ewarn ""
	fi
...
src_install() {
	...
	if use pdf; then
		pushd "${D}"/usr/libexec/cups/filter > /dev/null
			rm pdftops imagetoraster commandtoescpx commandtopclx \
				rastertopclx rastertoescpx texttops imagetops
		popd > /dev/null
		pushd "${D}"/usr/libexec/cups/backend/ > /dev/null
			rm serial parallel
		popd > /dev/null
		rm "${D}"/usr/share/cups/data/testprint
