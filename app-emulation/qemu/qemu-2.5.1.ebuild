...
SLOT="0"
IUSE="accessibility +aio alsa bluetooth +caps +curl debug +fdt \
glusterfs gnutls gtk gtk2 infiniband iscsi +jpeg \
kernel_linux kernel_FreeBSD lzo ncurses nfs nls numa opengl \
pax +pin-upstream-blobs +png pulseaudio python \
rbd sasl +seccomp sdl sdl2 selinux smartcard snappy spice ssh \
static static-softmmu static-user systemtap \
tci test +threads usb usbredir +uuid \
...
src_test() {
	if [[ -n ${softmmu_targets} ]]; then
		...
		use pax && pax-mark sxr */qemu-system-*
		emake -j1 check
...
src_install() {
	...
	use pax && pax-mark sxr "${softmmu_bins[@]}" "${user_bins[@]}"
	popd >/dev/null
